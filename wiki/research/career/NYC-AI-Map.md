---
category: career
type: list
scope: USA (New York)
pillar: Cross-pillar
tags:
- list
- Map
- USA
- NYC
- NewYork
about: Deep-dive list of 15+ New York City AI leaders focusing on Vertical AI (Finance,
  Legal, Pharma).
---
# USA AI Map: New York City

> [!IMPORTANT] Key Takeaway
> **Why this matters:** New York City is the primary hub for Vertical AI applied to high-value sectors like Finance, Legal, and Pharmaceuticals.
> **How to use it:** Map NYC leaders in autonomous financial modeling and clinical trials to understand vertical-specific AI adoption.
> **Informs:** [[Global-AI-Index]], [[USA-AI-Map]]

## 🤖 Agentic AI & Automation
- [[Rogo]] (Autonomous Financial Modeling)
- **Clay AI** (Autonomous GTM / Sales)
- **Dash0** (Agent Observability & Infra)
- **Toggle AI** (Autonomous Market Analysis)

## 🎯 Vertical AI & Specialized Domain
- [[Hebbia]] (Matrix - Financial Asset Analysis)
- [[Formation-Bio]] (AI-native Clinical Trials)
- [[MarqVision]] (IP & Brand Integrity)
- **Tempus AI** (Precision Oncology)
- **Kensho** (Financial Decisioning / S&P Global)
- **Traba** (Industrial Staffing / Supply Chain)
- **Viam** (Robotics Software for Smart Cities)
- **Teragonia** (Private Equity Value Orchestration)

## 🔗 Connections
- [[Global-AI-Index]]
- [[USA-AI-Map]]
- [[AI-Industry-Map-2026]]
